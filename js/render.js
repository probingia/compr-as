...
import { normalizar, agruparPorCategoria, capitalizar, getProductosFiltrados, limpiarContenedor } from './utils.js';
...
const crearElementoProducto = (p, isExport = false) => {
    ...
    const checkId = isExport ? `export-check-${p.id}` : `check-${p.id}`;
    checkbox.id = checkId;
    ...
    if (!isExport) {
        ...
        deleteButton.id = `btn-eliminar-${p.id}`;
...
export const renderSugerencias = (sugerencias) => {
        limpiarContenedor(dom.sugerenciasDataList);
...
const crearFilaSeleccionHTML = (p, options) => {
    ...
    const checkboxHtml = options.type === 'import'
        ? `<input class="form-check-input import-item-check" type="checkbox" id="import-check-${options.index}" data-index="${options.index}" checked>`
        : `<input class="form-check-input export-item-check" type="checkbox" id="export-select-check-${p.id}" value="${p.id}" checked>`;
...
export const renderImportPreview = (productos, errores) => {
        ...
        content += `
            ...
            <div class="table-responsive">
                <table class="table table-sm table-striped table-hover">
                    <thead>
                        <tr>
                            <th></th><th>Producto</th><th>Cant.</th><th>Precio</th><th>Prioridad</th><th>Tienda</th><th>Notas</th><th>Comprado</th>
                        </tr>
                    </thead>
                    <tbody>${filas}</tbody>
                </table>
    ...
    if (errores.length > 0) {
        const filasError = errores.map(e => `
            <tr>
                <td><i class="bi bi-exclamation-triangle-fill text-danger"></i></td>
                <td colspan="2"><small class="text-muted">Línea ${e.num}: <code>${e.contenido}</code></small></td>
                <td colspan="5"><small class="text-danger">${e.error}</small></td>
            </tr>
        `).join('');
        errorContent = `
            <h5 class="mt-4"><i class="bi bi-x-octagon-fill text-danger"></i> ${errores.length} Líneas con Errores</h5>
            <div class="table-responsive">
    ...
    limpiarContenedor(dom.importPreviewBody);
    dom.importPreviewBody.innerHTML = content + errorContent;
...
export const renderExportSelection = () => {
    ...
    if (productosAMostrar.length > 0) {
        ...
        limpiarContenedor(dom.exportSelectionBody);
...
export const renderImportResult = (productosImportados) => {
    ...
    if (productosImportados.length > 0) {
        ...
        limpiarContenedor(dom.importResultBody);
    dom.importResultBody.innerHTML = content;
};

export const renderGestion = () => {
    // Renderizar Categorías
        limpiarContenedor(dom.listaCategoriasUl);
    getCategories()
        ...
        .forEach(c => {
            ...
            const editButton = document.createElement('button');
            editButton.id = `gestion-cat-editar-${c.id}`;
            ...
            const deleteButton = document.createElement('button');
            deleteButton.id = `gestion-cat-eliminar-${c.id}`;
    ...
    // Renderizar Tiendas
        limpiarContenedor(dom.listaTiendasUl);
    getStores()
        .sort((a, b) => a.nombre.toLowerCase().localeCompare(b.nombre.toLowerCase()))
        .forEach(t => {
            const item = document.createElement('li');
            ...
            const div = document.createElement('div');
            const editButton = document.createElement('button');
            editButton.id = `gestion-tienda-editar-${t.id}`;
            ...
            const deleteButton = document.createElement('button');
            deleteButton.id = `gestion-tienda-eliminar-${t.id}`;
    ...
    const listaFiltrada = state.listaAutocompletado.filter(p => p.toLowerCase().includes(filtro));
        ...
        limpiarContenedor(dom.listaAutocompletadoUl);
    listaFiltrada.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())).forEach((p, index) => {
        const item = document.createElement('li');
        item.className = 'list-group-item d-flex justify-content-between align-items-center';
        ...
        const div = document.createElement('div');
        const editButton = document.createElement('button');
        editButton.id = `gestion-autocompletado-editar-${index}`;
        ...
        const deleteButton = document.createElement('button');
        deleteButton.id = `gestion-autocompletado-eliminar-${index}`;
