import { state, getCategories, getStores, addCategory, editCategory, deleteCategory, addStore, editStore, deleteStore, addProduct, updateProduct, deleteProduct, deleteAllProducts, toggleProductStatus, removeFromAutocomplete, editAutocompleteItem, addImportedProduct, setSortMode, toggleHidePurchased } from './state.js';
import { dom } from './dom.js';
import { capitalizar, normalizar, getProductosFiltrados, validarProducto } from './utils.js';
import { render, renderSugerencias, renderSelect, renderProductos, renderGestion, renderImportPreview, renderImportResult, renderExportSelection, renderTotales } from './render.js';
...
const actualizarSimboloPrecio = (unidad, elementoSimbolo) => {
    if (!elementoSimbolo) return;
    let simbolo = '€';
    if (unidad && unidad !== 'g') {
        simbolo = `€/${unidad}`;
    } else if (unidad === 'g') {
...
const importManager = (() => {
    ...
    return {
        setProductos: (productos) => {
            productosAImportar = productos;
        },
        getProductos: () => {
            return productosAImportar;
        },
        clearProductos: () => {
            productosAImportar = [];
        }
...
const abrirModalEdicion = (producto) => {
    ...
    dom.editModalBody.innerHTML = `
        ...
        <div class="mb-3">
            <input type="text" id="${idPrefix}nombre-producto" class="form-control" value="${producto.nombre}" placeholder="Nombre">
        </div>
        <div class="row g-3 mb-3">
            <div class="col-md-6">
                <input type="number" id="${idPrefix}cantidad-producto" class="form-control" value="${producto.cantidad}" placeholder="Cantidad">
            </div>
            <div class="col-md-6">
        ...
        <div class="mb-3">
            <div class="input-group">
                <input type="number" id="${idPrefix}precio-producto" class="form-control" value="${producto.precioUnitario || ''}" placeholder="Precio" min="0" step="1">
                <span id="${idPrefix}precio-unidad-simbolo" class="input-group-text">€</span>
            </div>
        </div>
        <div class="mb-3">
            <select id="${idPrefix}categoria-producto" class="form-select" aria-label="Categoría"></select>
        </div>
        <div class="mb-3">
            <select id="${idPrefix}tienda-producto" class="form-select" aria-label="Tienda"></select>
        </div>
        <div class="mb-3">
            <select id="${idPrefix}prioridad-producto" class="form-select" aria-label="Prioridad">
        ...
        <div class="mb-3">
            <textarea id="${idPrefix}notas-producto" class="form-control" rows="1" placeholder="Notas">${producto.notas}</textarea>
        </div>
    `;
    
    const catSelect = dom.editModalBody.querySelector(`#${idPrefix}categoria-producto`);
    if (catSelect) {
        renderSelect(catSelect, getCategories(), 'Elige categoría...');
        catSelect.value = producto.categoriaId;
    }

    const tiendaSelect = dom.editModalBody.querySelector(`#${idPrefix}tienda-producto`);
    if (tiendaSelect) {
        renderSelect(tiendaSelect, getStores(), 'Elige tienda...');
        tiendaSelect.value = producto.tiendaId;
    }

    const unidadSelect = dom.editModalBody.querySelector(`#${idPrefix}unidad-producto`);
    const simboloPrecio = dom.editModalBody.querySelector(`#${idPrefix}precio-unidad-simbolo`);

    if (unidadSelect && simboloPrecio) {
        actualizarSimboloPrecio(unidadSelect.value, simboloPrecio);
        unidadSelect.addEventListener('change', (e) => {
            actualizarSimboloPrecio(e.target.value, simboloPrecio);
        });
    }
...
const handleGuardarEdicion = async () => {
    ...
    const cantidadInput = document.getElementById(`${idPrefix}cantidad-producto`).value.replace(',', '.');

    const validacion = validarProducto(nombreInput, cantidadInput);

    if (validacion.error) {
        return mostrarNotificacion(validacion.error, 'error');
    }

    const precioValue = document.getElementById(`${idPrefix}precio-producto`).value.replace(',', '.');
    const productData = {
        nombre: validacion.nombre,
        cantidad: validacion.cantidad,
        unidad: document.getElementById(`${idPrefix}unidad-producto`).value,
        precioUnitario: parseFloat(precioValue) || 0,
...
const handleAnadirProducto = async () => {
    ...
    const cantidadInput = dom.cantidadProductoInput.value.replace(',', '.');
    ...
    const precioValue = dom.precioProductoInput.value.replace(',', '.');
    const productData = {
        nombre: validacion.nombre,
        cantidad: validacion.cantidad,
    ...
    if (result.success) {
        render();
        mostrarNotificacion(result.message, 'success');
        [dom.nombreProductoInput, dom.cantidadProductoInput, dom.notasProductoInput, dom.precioProductoInput].forEach(i => i.value = '');
        [dom.categoriaProductoSelect, dom.tiendaProductoSelect].forEach(s => s.selectedIndex = 0);
        dom.prioridadProductoSelect.value = 'baja';
        dom.cantidadProductoInput.value = '1';
    } else {
        mostrarNotificacion(result.message, 'error');
    }
};
...
const handleOrdenar = () => {
    ...
    const newMode = ordenes[nextIndex];
    setSortMode(newMode);
...
const handleImportarTxt = (event) => {
    ...
    reader.onload = (e) => {
        ...
        importManager.setProductos(productosParseados);
...
const handleConfirmarImportacion = async () => {
    ...
    const productosAImportar = importManager.getProductos();
...
const handleOcultarComprados = () => {
    const ocultos = toggleHidePurchased();
    renderProductos();
    mostrarNotificacion(ocultos ? 'Productos comprados ocultos.' : 'Mostrando todos los productos.', 'success');
};
...
export function setupEventListeners() {
    ...
    dom.unidadProductoSelect.addEventListener('change', (e) => {
        actualizarSimboloPrecio(e.target.value, dom.precioUnidadSimbolo);
    });
