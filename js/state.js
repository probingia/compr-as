...
export const setSortMode = (mode) => {
    const validModes = ['categoria', 'alfa', 'coste'];
    if (validModes.includes(mode)) {
        state.modoOrden = mode;
        if (mode === 'coste') {
            state.costeOrden = 'asc'; // Default a asc cuando se cambia a coste
        }
    } // No se guarda el estado aquí, es un cambio de UI efímero
};

export const toggleHidePurchased = () => {
    state.compradosOcultos = !state.compradosOcultos;
    // No se guarda el estado aquí, es un cambio de UI efímero
    return state.compradosOcultos;
};